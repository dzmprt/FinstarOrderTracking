...
public class BaseRepositoryTests
{
    ...
    public async Task FirstOrDefaultAsync_ShouldReturnEntity_WhenExists()
    {
        ...
        var result =
            await repository.FirstOrDefaultAsync(o => order.OrderNumber == o.OrderNumber, CancellationToken.None);
        Assert.NotNull(result);
        Assert.Equal(order.OrderNumber, result.OrderNumber);
    }
    ...
    public async Task SearchAsync_ShouldReturnEntities()
    {
        ...
        var result = await repository.SearchAsync<Order>(c => c.Description.StartsWith("SearchAsync Test"), null, null,
